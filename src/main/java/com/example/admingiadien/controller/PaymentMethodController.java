...
public class PaymentMethodController {
    ...
    public String editPaymentMethod(@PathVariable("id") Long id, @ModelAttribute("paymentMethod") PaymentMethodsDTO paymentMethodDTO, RedirectAttributes redirectAttributes) {
        try {
            // Lấy thông tin phương thức thanh toán dựa trên ID
            PaymentMethodsDTO existingPaymentMethod = paymentMethodService.findPaymentMethodById(id);

            // Kiểm tra xem phương thức thanh toán có tồn tại hay không
            if (existingPaymentMethod == null) {
                // Nếu không tìm thấy, xử lý trường hợp này và thông báo cho người dùng
                redirectAttributes.addFlashAttribute("errorMessage", "Failed to edit payment method: Payment method not found with id: " + id);
                return "redirect:/admin/paymentmethod";
            }
            ...
            // Đảm bảo chỉ người dùng hiện tại mới có thể sửa phương thức thanh toán
            if (!currentUserId.equals(existingPaymentMethod.getUserId())) {
                // Nếu không phải là người dùng hiện tại, không cho phép sửa và thông báo cho người dùng
                redirectAttributes.addFlashAttribute("errorMessage", "Failed to edit payment method: You don't have permission to edit this payment method");
                return "redirect:/admin/paymentmethod";
            }
